...
def informes_covid(datos: str) -> dict:
    if datos.endswith('.csv'):
      ...
      mas_contagios = df["País de procedencia"].mode()
      mas_contagios = mas_contagios[0]
      # Filtra los registros que sean del país con más contagios
      filtro = df.loc[:,"País de procedencia"] == mas_contagios
      contagios = df[filtro]
      ...
      respuesta: dict = {
               ...
               "cantidad_contagios": contagios["País de procedencia"].count(),
